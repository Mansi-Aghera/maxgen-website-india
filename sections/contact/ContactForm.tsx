...
function Social({
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="flex gap-3 items-start group">
      <div
        className="w-9 h-9 flex items-center justify-center rounded-full transition-transform group-hover:scale-110"
        style={{ background: "var(--color-accent)", color: "white" }}
      >
...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newErrors.full_name || newErrors.email || newErrors.phone || newErrors.message) {
      return
    }
    ...
    try {
      const res = await fetch(API.contact, {   // ✅ BASE URL USED
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          full_name: form.full_name,
    ...
    <Section className="py-16 md:py-24">
      ...
      <motion.div {...stagger} className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-start">
        ...
        <motion.div {...fadeUp} className="space-y-6">
          ...
          <div className="space-y-4 pt-2">
            <Social icon={<Facebook size={18} />} title="Follow us on Facebook" subtitle="Follow on Facebook to get latest details of Maxgen" href="https://www.facebook.com/maxgentechnologies" />
            <Social icon={<Instagram size={18} />} title="Follow us on Instagram" subtitle="Follow on Instagram to get latest details of Maxgen" href="https://www.instagram.com/maxgen_technologies/" />
            <Social icon={<Linkedin size={18} />} title="Follow us on LinkedIn" subtitle="Follow on LinkedIn to get latest details of Maxgen" href="https://www.linkedin.com/company/maxgen-technologies" />
          </div>
        </motion.div>
