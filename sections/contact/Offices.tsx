...
export default function Offices() {
  ...
  useEffect(() => {
    const fetchOffices = async () => {
      try {
        const res = await fetch("https://maxproject.pythonanywhere.com/offices/")
        ...
        const formatted = data.map((item: any) => ({
          ...
          email: item.email,
          inquiry: item.general_inquiry,
          contact: item.contact_numbers?.join(" / "),
          whatsapp: item.whatsapp_numbers?.join(" / "),
          extra: item.extra,
        }))
      ...
      >
        {offices.map((office, i) => (
          <motion.div
            key={i}
            {...fadeUp}
            className="relative bg-white border px-5 sm:px-6 pt-10 pb-6 text-center"
            style={{
              borderColor: "var(--color-border)",
              borderRadius: "var(--radius-lg)",
              boxShadow: "var(--shadow-md)",
          ...
          >
            ...
            <div className="mt-4 space-y-1 text-sm break-words">
              ...
              {office.extra && (
                <p className="mt-1 text-xs opacity-80">{office.extra}</p>
              )}
            </div>
          </motion.div>
        ))}
